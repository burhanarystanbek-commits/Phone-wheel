...
class MainActivity : Activity(), SensorEventListener {
    ...
    private fun tabBtn(lbl: String, selected: Boolean) = Button(this).apply {
        text = lbl
    ...
    private fun buildUi() {
        ...
        topRow.addWidget(statusText, lp(0, WRAP, 1f))
    ...
    // Helper because LinearLayout doesn't have addWidget
    private fun LinearLayout.addWidget(v: View, lp: ViewGroup.LayoutParams) = addView(v, lp)
    ...
    private fun setConnMode(usb: Boolean, usbBtn: Button, wifiBtn: Button) {
        ...
        usbBtn.background = roundRect(if (usb) Color.parseColor("#1a213a") else Color.parseColor("#1e2536"), dp(10).toFloat())
        wifiBtn.setTextColor(if (!usb) Color.parseColor("#7c6fff") else Color.parseColor("#6b7394"))
        wifiBtn.background = roundRect(if (!usb) Color.parseColor("#1a213a") else Color.parseColor("#1e2536"), dp(10).toFloat())
    ...
    private fun connect() {
        ...
        socket = client.newWebSocket(Request.Builder().url(url).build(), object : WebSocketListener() {
            override fun onOpen(ws: WebSocket, response: Response) {
                connected = true
                ws.send(JSONObject().put("type","hello").put("name", android.os.Build.MODEL)
    ...
    private fun startSendLoop() {
        handler.post(object : Runnable {
            override fun run() {
                if (connected) {
                    try {
                        // FIX: map pedals 0..1 → 0.5..1.0 so vJoy axis rests at CENTER
                        // This prevents cursor from moving when pedals are not pressed
